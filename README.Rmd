...
Load `ERPEDA`.
```{r pkgs_ERPEDA,message=FALSE}
library(ERPEDA)
```

We demonstrate the graphical capabilities of `ERPEDA` with these real data set `DirectedForgetting`. Download data `DirectedForgetting` from [website](https://goo.gl/NsB5pX) or directly use the build-in one.
